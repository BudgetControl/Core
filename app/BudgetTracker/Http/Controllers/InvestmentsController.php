...
use App\BudgetTracker\Services\InvestmentsService;
use League\Config\Exception\ValidationException;
use App\BudgetTracker\Services\ResponseService;

class InvestmentsController extends EntryController
{
	use Paginate;
	...
	public function store(Request $request): \Illuminate\Http\Response
	{
		try {
			$service = new InvestmentsService();
			$service->save($request->toArray(), EntryType::Investments);
	...
	{
		try {
			$service = new InvestmentsService($uuid);
			$service->save($request->toArray(), EntryType::Expenses);
			return response('All data stored');
		} catch (\Exception $e) {
			return response($e->getMessage(), 500);
		}
